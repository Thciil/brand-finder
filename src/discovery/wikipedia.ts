...
export async function discoverCompanies(
  ...
  const insertStmt = db.prepare(`
    INSERT OR IGNORE INTO companies (name, wikipedia_url, website_url, category, region, status)
    VALUES (?, ?, ?, ?, ?, 'unqualified')
  `);
...
export function getUnqualifiedCompanies(): Array<{
  id: number;
  name: string;
  ...
  return db
    .prepare(
      `SELECT id, name, wikipedia_url, website_url FROM companies WHERE status = 'unqualified'`
    )
    .all() as Array<{
    id: number;
    name: string;
